function [ FracErr, MaxErr,SOCerr,rmse ] = fun2(x,averageocp,N,Up,Un,p_soc,n_soc,range)
...
y=[SOCexp,SOCextrap];
SOCmodel=interp1(x, y(index),ocv(range(end):end));
SOCe=SOCmodel-SOCexp(range(end):end);
FracErr=sum(ge(abs(ocpe),.005)/N);
MaxErr=max(abs(ocpe));
SOCerr=max(abs(SOCe));
F=FracErr+0*1*max(abs(ocpe))+0*max(abs(SOCe))+0*rmse;% Returns Max of error
end
